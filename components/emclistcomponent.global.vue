...
<template>
  <div v-if="props.OutputDataForList">
    ...
    <!-- ================= LOADER ================= -->
    <VSkeletonLoader type="table-row-divider, table-tbody" :loading="loading">

      <!-- ================= TABLE VIEW ================= -->
      <VDataTable v-if="listviewmode === 'table'" v-model="SelectedRows" :headers="props.ListHeaders"
        :items="props.OutputDataForList" item-value="_id" class="truncate-table" density="comfortable"
        disable-pagination hide-default-footer :items-per-page="-1" fixed-header return-object show-select />

      <!-- ================= CARD VIEW ================= -->
      <VRow v-if="listviewmode === 'card'" class="ma-1" dense>
        <VCol v-for="item in props.OutputDataForList" :key="item._id" cols="12" sm="6" md="3">
          <VCard elevation="2" border>

            <!-- HEADER ROW -->
            <VCardTitle class="d-flex align-center justify-space-between">
              <span :class="headerDisplayMap.get(item._id)?.isEmpty
                ? 'text-medium-emphasis text-caption font-italic'
                : 'font-weight-bold'">
                {{ headerDisplayMap.get(item._id)?.value }}
              </span>

              <VCheckbox :model-value="isSelected(item)" density="compact" hide-details @click.stop
            ...
            <VCardText class="py-1 px-2">
              <VRow v-for="header in props.ListHeaders.slice(1)" :key="header.key" dense class="py-1">
                <VCol cols="4" class="text-caption text-grey pa-0">
                  {{ header.title }}
                </VCol>

                <VCol cols="8" class="font-weight-medium text-truncate pa-0" :title="getValueByPath(item, header.key)">
      ...
      <VContainer fluid density="compact" class="h-10 pa-1 ma-0">
        <VRow justify="start" density="compact" class="align-center">

          <VCol v-if="SelectedRows?.length > 0" cols="2" class="pa-4 ma-0">
          ...
          <VCol cols="2" align="right">
            <VSelect v-model="PaginationParameters.Limit" :items="[1, 2, 5, 10, 50, 100]" density="compact"
              style="inline-size: 60%;" @update:model-value="
          ...
          <VCol cols="4">
            <VPagination v-model:model-value="PaginationParameters.CurrentPage" density="compact"
              :length="PaginationParameters?.TotalPages" @update:model-value="emit('selectionchanged')" />
          </VCol>
