...
export const getComputedNavLinkToProp = computed(() => (link: NavLink) => {
  ...
  if (link.to) {
    // Guard against undefined route names
    if (typeof link.to === 'string' && link.to !== 'undefined') {
      props.to = { name: link.to }
    } else if (typeof link.to === 'object' && link.to !== null) {
      // Validate route object doesn't have undefined name
      if (link.to.name && link.to.name !== 'undefined') {
        props.to = link.to
      } else if (link.to.path && link.to.path !== '/undefined') {
        props.to = link.to
      }
    }
  } else if (link.href) {
