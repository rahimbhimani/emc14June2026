...
export default defineEventHandler(async event => {
  ...
  if (body.action.toUpperCase() !== 'REFDATA') {
    ...
    collection = cn.collection(body.master === 'ScreenConfigure' ? 'emcFormDesign' : getProperty('Table Name'))
  }
  ...
  case 'INSERT':
    ...
    let resw = {}
    if (body.data._id) {
      // // console.log(.log('within insert1')

      const lRValue = await updatetData(collection, body, resw)
      // // // console.log(.log('within insert2', lRValue)
      return lRValue
      break
    }
    else {
      if (body.master === 'ScreenConfigure') {
        ...
        // // console.log(.log('within insert2')
...
async function retreiveDTData(vId = '', vSearchWhat = '', vPropertyToLookFor = '', vControlDepth = 1, vretreiveData = false) {
  // // console.log(.log('insideretreiveDTData', vId, vSearchWhat, vPropertyToLookFor)
...
function getValueByPath(obj, path) {
  ...
  const field = parts.pop();                // last attribute after the dot
  const target = parts.reduce((acc, part) => acc && acc[part], obj);

  // console.log(.log("getValueByPath", parts, field, target)
...
async function getGroupData(cn, body) {
  ...
  switch (body.data.grouptype?.toUpperCase()) {
    case 'LOCATION':
      return await getLocationData(cn, body)
    default:
      const lGroupDefination = await cn.collection('emcGroupDefination').find({GroupCode : body.data.grouptype}).toArray()
...
// ])
    ...
    const lTableName = await retreiveDTData(body.data.id, ltxtControls, 'SearchTableName', lControlDepth, true)
    console.log('BeforelWhereClause0', body.data.id)
    const lColumnNames = await retreiveDTData(body.data.id, ltxtControls, 'SearchColumnNames', lControlDepth)
    console.log('BeforelWhereClause1', lColumnNames)
    let lWhereClause = await retreiveDTData(body.data.id, ltxtControls, 'WhereClause', lControlDepth)
    ...
    let lsortClause = await retreiveDTData(body.data.id, ltxtControls, 'SortBy', lControlDepth)
    ...
    const results = await cn
      ...
      .find(lWhereClause, { projection: { [lColumnNames]: 1, [lOtherColumn]: 1, _id: 1 } })
      .sort(lsortClause)
      .toArray()
  ...
  const pipeline = [
    ...
    },
    {
      $unionWith: {
        ...
        pipeline: [
          {
            $match: { "TabBasicDetails.TbBasicDetails.Name": { $regex: regex } }
          },
          {
            $project: {
              _id: 0,
...
async function getDataForControl(cn, body) {
    ...
    let lTableName = await retreiveDTData(body.data.id, ltxtControls, 'SearchTableName', lControlDepth, true)
    console.log('lTableName33', lTableName)

if (!lTableName || lTableName !== '') {
  //This is to handle hardcoded values within Group Data control
  console.log('lTableName333', lTableName.includes("{"),lTableName.includes("}"))
...
async function getDetail(collection, cn, body) {
  ...
  try {
    ...
    const objectIds = body.data.map(item => new mongoose.Types.ObjectId(item._id))
...
async function updatetData(collection, body, res) {
  ...
  try {
    ...
    if(body.master === 'ScreenConfigure'){
      ...
      lemcFormDesign.FormDTObjects.validationSchema = extractZodJsonFromDTObjects(lemcFormDesign.FormDTObjects.board[0].Controls)
      ...
      lemcFormDesign.FormRTObjects.validationSchema =lemcFormDesign.FormDTObjects.validationSchema
...
async function GetControlsData(collection, cn, body) {
  ...
  // cursor = collectionMaster.find().filter({ 'FormParameters.Name': body.master.value })
  cursor = collectionMaster.find().filter({ $or: [{'FormParameters.master': body.master},{'FormParameters.Name': body.master} ]})
...
async function GetLifeCycleForMaster(vmaster){
    ...
    let cursor
    cursor = collectionMaster.find().filter({ $or: [{'master': vmaster},{'Name': vmaster} ]})
...
async function populateDesignTimeData(vbody) {
  try {
    ...
    mDTObject = await cn.collection('emcFormDesign').find({
      'FormDTObjects.board': {
        $elemMatch: {
          'ControlName': lMaster
        }
...
function getProperty(vProperty){
    ...
    const property = lDTObject.controlProperties.find(prop => prop.propertyTitle === vProperty)
      ...
      if (hasUniqueKey) {
        result.push({
          ...
          controlProperties: control.controlProperties.filter(
            (prop: any) => prop.propertyTitle === "UniqueKey" || prop.propertyTitle === "ColumnName"
          ),
...
async function checkUniqueattributes(vbody) {
  ...
  const collectionMaster = cn.collection(body.master === 'ScreenConfigure' ? 'emcFormDesign' : await getProperty('Table Name'))
  ...
  const controls = findControlsWithUniqueKey(mDTObject[0].FormDTObjects.board[0].Controls) //mDTObject[0].FormDTObjects.board[0].Controls
  // console.log(.log('Starting checkUniqueattributes3', controls)
  // Collect UniqueKey values
  const lUniqueStr = controls
    .map(obj => obj.controlProperties.find(e => e.propertyTitle === 'UniqueKey')?.data)
    .filter(Boolean)
    .join(',')

  const uniqueArr = [...new Set(lUniqueStr.split(',').map(val => val.trim()).filter(Boolean))]
  ...
  for (const uniqueItem of uniqueArr) {
    ...
    for (const item of controls) {
      ...
      const uniqueKeys = controlProps.find(e => e.propertyTitle === 'UniqueKey')?.data?.split(',').map(s => s.trim())
      // console.log(.log('Starting checkUniqueattributes6', uniqueKeys)
      if (uniqueKeys?.includes(uniqueItem)) {
        const columnName = controlProps.find(e => e.propertyTitle === 'ColumnName')?.data
        // console.log(.log('Starting checkUniqueattributes7', columnName)
        if (columnName) {
          const ldata = item.dataPath.replace('FormName.', '').split('.').reduce((acc, key) => acc?.[key], body.data)
    ...
    if (body.data._id) {
      lObjToValidate['_id'] = { $ne: new mongoose.Types.ObjectId(body.data._id) }
    }
...
async function GetListData(collection, cn, body) {
  ...
  const collectionMaster = cn.collection('emcFormDesign')
  const lretObj = { FormData: { validationSchema: null, FormParameters: null, UserEntryObjects: null, ListHeaders: null, FormRTSearchObjects: null, GridData: null, SearchCriteriaAttributesData: null, PaginationOutPutData: null, error: { errorCode: '', errorDesc: '' } } }
  let lFormParameters = { ListHeaders: { headers: null } }
  //// // // console.log(.log('testing 0 ')
  if (body.retreiveConfData === true) {
    //// // // console.log(.log('inside body.retreiveConfData 1 ')
    lFormParameters = collectionMaster.find({ $or: [{'FormParameters.master': body.master},{'FormParameters.Name': body.master} ]})
    ...
    lretObj.FormData.FormParameters = { Title: lFormParameters.FormParameters.Title, Description: lFormParameters.FormParameters.Description }
    
    lretObj.FormData.ListHeaders = lFormParameters['ListHeaders']
    
    lretObj.FormData.FormRTSearchObjects = lFormParameters['FormRTSearchObjects']
    
    lretObj.FormData.validationSchema = lFormParameters.FormRTObjects?.validationSchema
  ...
  cursor = collection.find(lGridDataWhereClause).limit(lLimit).skip(lstartIndex)
  

  const lTotalRecordCount = await collection.countDocuments(lGridDataWhereClause)
...
function returnConditionWithObject(vCond: any) {
  ...
  const lInput = vCond.Attribute.datatype === 'number' ? Number(vCond.Input) : vCond.Input
...
export function extractZodJsonFromDTObjects(controls: any[]): any {
  ...
  controls.forEach(control => {
    ...
    if (control.controlProperties) {
      control.controlProperties.forEach(prop => {
        if (prop.propertyTitle === "DataType") {
          controlSchema.type = prop.data === "number" ? "number" :
                               prop.data === "boolean" ? "boolean" : "string";
        }

        if (prop.propertyTitle === "Mandatory") {
          controlSchema.required = prop.data?.value === "true";
          // controlSchema.allowNull = !controlSchema.required
          if(controlSchema.required === true && (controlSchema.type === "DropDown" || controlSchema.type === "AutoComplete" || controlSchema.type === "GroupData")){
    ...
    if(control.controlType === "RateDefinition"){
      // console.log("RateDefinitioncontrol123999", control)
      controlSchema.type = "object"
      controlSchema.ignore = true
    }
    if(control.controlType === "DropDown" || controlSchema.type === "AutoComplete" ){
      console.log("dropdown124", control.controlProperties.filter(e=>e.propertyTitle === 'Multiselect')[0]?.data)
      controlSchema.type = control.controlProperties.filter(e=>e.propertyTitle === 'Multiselect')[0]?.data === 'true' ? "Array" : "object"
      controlSchema._id = {
              "required": false,
              "type": "string",
              "min": 1,
...
controlSchema.Type = {
  ...
  controlSchema.rows = {
    ...
    if (control.Controls && control.Controls.length > 0) {
      ...
      if (control.controlType === "GridTable") 
        controlSchema.type = "Array"
