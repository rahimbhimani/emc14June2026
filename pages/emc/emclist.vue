...
async function btnMoveToSave() {
  ...
  if (success === true) {
    if (lObj?.data.value.insertedID)
      muserDataStore.data.FormData.UserEntryObjects.FormName._id = lObj?.data.value.insertedID

    await informUser('success', `Data successfully ${lObj?.data.value.insertedID ? 'created' : 'updated'}. `)
  } else if (success === false) {
    // alert(lObj?.data.value.insertedID)
    await informUser('error', `${lObj?.data.value.insertedID || lObj?.data.value.insertedID === undefined ? 'Create' : 'Update'} action Failed ! \n ${lObj?.data.value.message}`)
  }
}
...
<!-- {{ mOutPutFormData?.FormData.validationSchema }} -->
  ...
  >
      ...
      <div  style="display: flex; align-items: baseline; gap: 6px;">
        ...
        <h5  class="text-truncate" style="margin: 0; color: darkgray; font-family: sans-serif; font-style: italic; font-weight: lighter;max-inline-size: 60%;">
    ...
    >
      <VIcon
        size="18px"
        color="green"
        :icon="mFormInputData.searchCriteria.length > 0 ? emcfilter : emcfilteroff"
      >
      </VIcon>
    </VBtn>
  </VToolbar>
  ...
  >
    <VContainer>
      ...
      >
        <VCol class="d-flex flex-wrap justify-space-between gap-4 mb-6" style="display: flex;align-items: center; justify-content: center; background-color: transparent; color: transparent;">
  ...
  <VCard
    ...
    class="mx-auto mt-2"  >
      ...
      <div v-show="mErrorOnPage === false && filterpanelexpanded === true " >
        <!-- {{ mFormInputData.searchCriteria }} -->
        <EmcSearchbar
          :searchAttributes="mOutPutFormData.FormData.FormRTSearchObjects"
  ...
  <!-- {{ muserDataStore.data.FormData }} -->
    ...
    <!-- {{ mFormInputData }} -->
      <!-- {{ muserDataStore.data.FormData.DataObject }} -->
    <VForm ref="formRef">
      <VCard  class="mt-2 mx-auto"  elevation="6">
    <component
